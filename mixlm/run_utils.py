...
def run_train(dataset, eval_dataset, hps, logdir, ps_device, task=0, master=""):
    with tf.variable_scope("model"):
        model = CLSTMDNN(hps, 'train', ps_device)
    ...
    with sv.managed_session(master, config=config) as sess:
        sw = tf.summary.FileWriter(logdir + '/' + "eval", sess.graph)
        ...
        data_iterator = dataset.iterate_forever(hps.batch_size * hps.num_gpus,
                                                hps.num_steps)
        eval_data_iterator = eval_dataset.iterate_forever(hps.batch_size * hps.num_gpus,
                                                          hps.num_steps)
        while not sv.should_stop():
            ...
            word_x, char_x, y, w = next(data_iterator)
            ...
            if should_run_profiler:
                ...
                fetched = sess.run(fetches, {model.word_x: word_x, model.char_x: char_x, model.y: y, model.w: w},
            ...
            if local_step < 10 or local_step % 20 == 0:
                ...
                # Run a eval in some step
                if local_step > 1000 and local_step % 500 == 0:
                    print("\nRun eval:")
                    # Training Set eval
                    loss_nom = 0.0
                    loss_den = 0.0

                    for i, (eval_word_x, eval_char_x, eval_y, eval_w) in enumerate(eval_data_iterator):
                        if i >= 100:
                            break

                        loss = sess.run(model.loss,
                                        {model.word_x: eval_word_x, model.char_x: eval_char_x, model.y: eval_y,
                                         model.w: eval_w})
                        loss_nom += loss
                        loss_den += w.mean()
                    ...
                    print("Results at %d: log_preplexity = %.3f perplexity = %.3f" % (
                        step, log_perplexity, np.exp(log_perplexity)))
                    print("\n")
...
def run_eval(dataset, hps, logdir, mode, num_eval_steps):
    ...
    with sess.as_default():
        while ckpt_loader.load_checkpoint():
            ...
            for i, (word_x, char_x, y, w) in enumerate(data_iterator):
                if i >= num_eval_steps:
                    break

                loss = sess.run(model.loss, {model.word_x: word_x, model.char_x: char_x, model.y: y, model.w: w})
                loss_nom += loss
                loss_den += w.mean()
